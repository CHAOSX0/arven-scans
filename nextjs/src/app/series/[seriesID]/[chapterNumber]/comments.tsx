...
export default function ChaptersComments({slug}: {slug: number}){
    ...
    useEffect(() => {
     supabase.from('comments').select().eq('chapterID', slug).then((res)=>{
    ...
    async function HandelSubmit(){
       ...
       if(user){
          ...
          const {error, data} = await supabase.from('comments').insert({text, author, votes: 0, authorUsername: user.user_metadata.username, authorAvatar: user.user_metadata.avatar, chapterID: slug, time: `${new Date().getTime()}`}).select()
          if(!error){
