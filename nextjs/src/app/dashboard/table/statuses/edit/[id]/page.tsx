...
export default function statusEdit({params: {id}}: {params: {id: string}}){
    ...
    useEffect(()=>{
        supabase.from('statuses').select('*').eq('id', id).then(res=>{
          if(res.error){
    ...
    async function Submit(e: React.SyntheticEvent): Promise<void>{
      e.preventDefault()
      const {error} = await supabase.from('statuses').update({text: formData.name, slug: formData.slug}).eq('id', parseInt(id)) 
      if(error){
    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-1">
        <div className="flex justify-between">
          <h3 className="text-3xl font-bold tracking-tight">Add New status</h3>
          <Link
