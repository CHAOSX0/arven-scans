...
export default function TypeEdit({params: {id}}: {params: {id: string}}){
    ...
    async function Submit(e: React.SyntheticEvent): Promise<void>{
      e.preventDefault()
      const {error} = await supabase.from('types').update({text: formData.name, slug: formData.slug}).eq('id', parseInt(id)) 
      if(error){
