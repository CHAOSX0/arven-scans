...
export default function GenreEdit({params: {id}}: {params: {id: string}}){
    const [isLoading, setIsLoading] = useState<boolean>(false)
    const [formData, setFormData] = useState<{name: string, slug: string }>({name: '', slug: ''})
    
    //$ e is the form submit event at line: 45
    useEffect(()=>{
        supabase.from('genres').select('*').eq('id', id).then(res=>{
          if(res.error){
            
            return
          }
          setFormData({name: res.data[0].text, slug: res.data[0].slug})
        })
    }, [])
    async function Submit(e: React.SyntheticEvent): Promise<void>{
      e.preventDefault()
      const {error} = await supabase.from('genres').update({text: formData.name, slug: formData.slug}).eq('id', parseInt(id)) 
      if(error){
    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-1">
        <div className="flex justify-between">
          <h3 className="text-3xl font-bold tracking-tight">Add New genre</h3>
          <Link
